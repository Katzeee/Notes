...
```bash
$ acme.sh --debug --installcert -d host.xacpve.fun --keypath /etc/pve/local/pve-ssl.key --fullchainpath /etc/pve/local/pve-ssl.pem --reloadcmd "systemctl restart pveproxy"
```
...
Then go to `http://<docker-ct-ip>:9000` to set up portainer.
...
services:
  qbittorrent:
    ...
    volumes:
      - /home/xac/qbittorrent/config:/config
