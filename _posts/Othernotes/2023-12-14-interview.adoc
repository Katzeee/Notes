...
正常求法就如上面所说，需要以一个三角形为基础，知道三点的法线方向与uv坐标得出，当然这部分不可能手算，太麻烦了。
...
+
--
Early-Z 是一种硬件级别的优化技术，它允许GPU在进行像素着色之前先执行深度测试。**early-z 会写入深度**
...
+
--
避免深度采样点分布不均匀时，在某一区间内由深度计算出的空间位置误差太大。
--
...
+
--
Tone mapping 是为了解决显示器没有办法显示HDR颜色，而在渲染过程中用的都是HDR贴图的问题，将HDR颜色映射到LDR色彩空间让其在显示屏上正常显示。通过这条tone mapping 曲线我们就可以将HDR亮度值转换到[0,1]
...
+
--
随机采样的效果比较差，容易有噪点，简单的均匀采样随机性太差，会出现规则的pattern
--
